...
#include "../../robot_algo/ik_panda.hpp"
#include "../../robot_algo/fk_panda.hpp"
#include "../../base/bounds.hpp"
#include "../../base/se3_space.hpp"
...
{
public:
    MoveL(const FKPandaPtr& fk, const IKPandaPtr& ik, const AllStateValidityCheckerPtr& checker, std::size_t dimension) :
        LocalPlannerBase(nullptr, checker), fk_(fk), ik_(ik), dimension_(dimension) {}
    
    // from: Jps to: se3 return: Jps
    std::optional<State> interpolateState(const State& startJps, const State& targetSe3, double time) const override
    {
        assert(startJps.size() == dimension_);
        assert(targetSe3.size() == 7);
        Eigen::Isometry3d startSe3 = fk_->getTcpPose(startJps);
        Eigen::Isometry3d pose = se3_.interpolateSe3(startSe3, se3_.state2Se3(targetSe3), time);
        return ik_->ik(startJps, pose);
    }
    // from: Jps to: se3 return: Jps
    std::optional<State> validInterpolateState(const State& startJps, const State& targetSe3, double time) const override
    {
        assert(startJps.size() == dimension_);
        assert(targetSe3.size() == 7);
    ...
    std::optional<Path> validInterpolatePath(const State& startJps, const State& targetSe3, double stepSize) const override
    {
        assert(startJps.size() == dimension_);
        assert(targetSe3.size() == 7);
